...
API REST del proyecto **Salva la Jama**. Sigue la estructura estándar de Laravel 11,
en modo API-only.
...
- Seeder con los datos demo del proyecto (Ana Salazar, admin, 3 establecimientos,
  6 paquetes, reserva `SLJ-4902` activa + 2 retiradas con reseña). Contraseña demo: `password`.

## Qué falta (siguientes sprints, por integrante)

Los endpoints de `establishments`, `packages`, `reservations`, `reviews`, `notifications`,
`admin` e `impact` todavía no están implementados —
quedan para los Sprints 1-5 según el reparto de módulos por integrante.
